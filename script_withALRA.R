...
if (!file.exists("results_withALRA")){   
  dir.create("results_withALRA")
  dir.create("results_withALRA/exploratory_analysis")
}
...
top10 <- head(VariableFeatures(scdown), 10)
...
for (i in c("seurat_clusters","donor","sex","condition")) {
  ...
  ggsave(paste0("results_withALRA/exploratory_analysis/umap_",i,".png"), plot = clusters, width = 12, height = 12)
}
...
ggsave("results_withALRA/exploratory_analysis/pct_condition_sex.png", plot = bar_plot, width = 3, height = 3)
...
ggsave("results_withALRA/exploratory_analysis/canonical_markers.png", plot = dot_plot, height = 5, width = 20)
...
scdown@meta.data <- scdown@meta.data %>%
  mutate(
    seurat_clusters = case_when(
      ...
      seurat_clusters == "14" ~ "Inh",
      seurat_clusters == "15" ~ "Exc"
    )
  )
...
saveRDS(scdown, file = "data/RDS/withALRA.rds")

#scdown <- readRDS("data/RDS/noALRA.rds")
scdown <- readRDS("data/RDS/withALRA.rds")
...
ggsave("results_withALRA/exploratory_analysis/ANNOTATED_umap.png", plot = umap_plot, height = 12, width = 12, dpi = 600)
...
rm(sub_scdown)
ggsave("results_withALRA/exploratory_analysis/ANNOTATED_umap_CT.png", plot = umap_plot, height = 12, width = 12, dpi = 600)
...
rm(sub_scdown)
ggsave("results_withALRA/exploratory_analysis/ANNOTATED_umap_DS.png", plot = umap_plot, height = 12, width = 12, dpi = 600)
...
for (i in c("donor","sex","condition")) {
  ...
  ggsave(paste0("results_withALRA/exploratory_analysis/ANNOTATED_pctbar_",i,".png"), plot = bar_plot, width = 5, height = 4)
}
...
dot_plot <- markersFunction()
ggsave("results_withALRA/exploratory_analysis/ANNOTATED_dotplot.png", plot = dot_plot, height = 4, width = 18)
...
ggsave("results_withALRA/exploratory_analysis/ANNOTATED_pct_composition.png", plot = pct_composition, height = 5, width = 6)
...
features_to_plot <- c("C1R","C1S","C2","C4A","C4B","C5","C5AR1","C6","C8G","CD59","CFI","FCN2")
View(DElist$Ast %>% filter(gene %in% features_to_plot))

all_DEdf <- do.call(rbind, DElist)
write.table(all_DEdf, file = "supplementary_tables/allDEdf_withALRA.tsv", sep = "\t", row.names = FALSE, quote = FALSE)
...
ast_cells <- subset(scdown, subset = seurat_clusters == "Ast")

features_to_plot <- DElist$Ast %>% 
  filter(DEG != "NO", chromosome_name == '21') %>% 
  arrange(p_val_adj) %>%
  head(50) %>% 
  pull(gene)

heatmap_plot <- DoHeatmap(ast_cells,
...
#rm(ast_cells)
ggsave("results_withALRA/ast_heatmap.png", plot = heatmap_plot, width = 2.5, height = 6, dpi = 600)
...
features_to_plot <- c("C1R","C1S","C2","C4A","C4B","C5","C5AR1","C6","C8G","CD59","CFI","FCN2","SLC1A2","SLC1A3")
...
for (i in c("Ast")) { # names(DElist)
  ...
  ggsave(paste0("results_withALRA/DE_", i, ".pdf"), plot = vln_plot, height = 4, width = 10)
}
...
AstCompSet <- GeneSet(c("C1R","C1S","C2","C4A","C4B","C5","C5AR1","C6","C8G","CD59","CFI","FCN2"), setName = "AstCompSet")
...
umap_feature <- FeaturePlot(
  ...
  labs(
    title = "C1R+C1S+C2+C4A+C4B+C5+\nC5AR1+C6+C8G+CD59+CFI+FCN2"
  ) +
  theme(
...
ggsave("results_withALRA/Ast_feature_umap.png", plot = umap_feature, height = 3.5, width = 5, limitsize = FALSE, dpi = 600)
...
ggsave("results_withALRA/compositionAllCelltypes.png", plot = comp_plot, height = 4, width = 4)
...
ggsave("results_withALRA/compositionAst.png", plot = comp_plot, height = 4, width = 10)
